...
describe('PostgresRefreshTokenRepository', () => {
  ...
  describe('rotate', () => {
    ...
    it('returns not_found when the old token does not exist', async () => {
      ...
      if (!firstCall) {
        throw new Error('Expected transaction query to be called');
      }
    ...
    it('detects reuse and revokes the token family', async () => {
      ...
      const secondCall = client.query.mock.calls[1];

      expect(secondCall).toBeDefined();

      if (!secondCall) {
        throw new Error('Expected family revocation query to be called');
      }

      const [familyQuery, familyParameters] = secondCall;
    ...
    it('rotates an active refresh token inside a transaction', async () => {
      ...
      if (!selectCall) {
        throw new Error('Expected refresh-token select query to be called');
      }

      const [selectQuery, selectParameters] = selectCall;
      ...
      if (!insertCall) {
        throw new Error('Expected replacement-token insert query to be called');
      }

      const [insertQuery, insertParameters] = insertCall;
      ...
      const revokeCall = client.query.mock.calls[2];

      expect(revokeCall).toBeDefined();

      if (!revokeCall) {
        throw new Error('Expected old-token revoke query to be called');
      }

      const [revokeQuery, revokeParameters] = revokeCall;
    ...
    it('preserves old device context when new context is absent', async () => {
      ...
      const [, insertParameters] = secondCall;
