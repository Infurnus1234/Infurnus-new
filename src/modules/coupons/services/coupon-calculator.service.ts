...
export class CouponCalculatorService {
  ...
  private validateCoupon(coupon: Coupon): void {
    ...
    if (!Number.isSafeInteger(coupon.minFareAmount)) {
      throw new Error(
        "minFareAmount must be a safe integer in paise",
      );
    }
    ...
    if (
      ...
      (!Number.isSafeInteger(coupon.usageLimit) ||
        coupon.usageLimit <= 0)
    ) {
    ...
    if (
      ...
      (!Number.isSafeInteger(coupon.perUserLimit) ||
    ...
    if (!Number.isSafeInteger(coupon.usageCount)) {
      throw new Error(
        "usageCount must be a safe integer",
      );
    }
