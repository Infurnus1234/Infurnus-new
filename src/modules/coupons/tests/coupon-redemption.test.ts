...
function createRepositoryMock() {
  return {
    ...
    [K in keyof CouponRepository]: ReturnType<
      typeof vi.fn
    >;
  };
}
...
describe("CouponRedemptionService", () => {
  ...
  describe("successful redemption", () => {
    it("redeems a valid coupon successfully", async () => {
      ...
      expect(
        repository.findByCodeForUpdate,
      ...
      expect(
        repository.countUserRedemptions,
      ).toHaveBeenCalledWith(
        coupon.id,
        redemption.userId,
        expect.anything(),
    ...
    it("stores the calculated financial snapshot", async () => {
      ...
      const createCall =
        repository.createRedemption.mock.calls[0]!;

      const input =
        createCall[0] as CreateCouponRedemptionInput;
  ...
  describe("per-user usage limit", () => {
    ...
    it("supports unlimited per-user usage", async () => {
      ...
      repository.countUserRedemptions.mockResolvedValue(
        1000,
      );
      repository.incrementUsage.mockResolvedValue(true);
      repository.createRedemption.mockResolvedValue(
  ...
  describe("calculation failure", () => {
    it("does not increment usage when calculation fails", async () => {
      ...
      await expect(
        ...
        "Minimum fare of 20000 paise is required for coupon SAVE20",
      );
  ...
  describe("transaction behavior", () => {
    it("executes the redemption inside withTransaction", async () => {
      ...
      expect(mockedWithTransaction).toHaveBeenCalledWith(
        expect.any(Function),
      );
    });
    ...
    it("propagates transaction failures", async () => {
      ...
      mockedWithTransaction.mockRejectedValue(
        transactionError,
      );
  ...
  describe("operation ordering", () => {
    it("locks, checks usage, increments, then creates redemption", async () => {
      ...
      repository.findByCodeForUpdate.mockImplementation(
        async () => {
          calls.push("lock");
          return coupon;
        },
      ...
      repository.countUserRedemptions.mockImplementation(
        async () => {
          calls.push("count");
          return 0;
        },
      ...
      repository.incrementUsage.mockImplementation(
        async () => {
      ...
      repository.createRedemption.mockImplementation(
        async () => {
          calls.push("create");
          return redemption;
        },
