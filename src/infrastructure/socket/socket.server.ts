    ...
    io.on('connection', (socket) => {
      ...
      socket.on('ride:join', async (rideId: unknown, ack?: SocketAck) => {
        try {
          ...
          ack?.({ success: true, data: { room: rideRoom(rideId) } });
        } catch {
          ack?.({
            success: false,
            error: { code: 'RIDE_ROOM_FORBIDDEN', message: 'Ride room access denied' },
          });
        }
      });

      socket.on('ride:leave', async (rideId: unknown, ack?: SocketAck) => {
        if (typeof rideId === 'string') await leaveRideRoom(socket, rideId);
        ack?.({ success: true });
      });
      ...
      socket.on('driver:location', async (payload: unknown, ack?: SocketAck) => {
        try {
          ...
          ack?.({
            success: false,
            error: { code: 'DRIVER_LOCATION_FORBIDDEN', message: 'Driver location update denied' },
          });
        }
      });
      ...
      socket.on('driver:accept', async (rideId: unknown, ack?: SocketAck) => {
        try {
          ...
          io.to(rideRoom(rideId)).emit('ride:driver_assigned', { ride });
          ack?.({ success: true, data: ride });
        } catch {
          ack?.({
            success: false,
            error: { code: 'RIDE_ACCEPTANCE_CONFLICT', message: 'Ride acceptance denied' },
          });
        }
      });
      ...
      socket.on('ride:status', async (payload: unknown, ack?: SocketAck) => {
        try {
          ...
          if (typeof rideId !== 'string') throw new Error('Ride identifier required');
          const profileId = await rideDependencies.driverService.profileForUser(userId!);
          ...
          io.to(rideRoom(rideId)).emit('ride:lifecycle_updated', { ride });
          ack?.({ success: true, data: ride });
        } catch {
          ack?.({
            success: false,
            error: { code: 'RIDE_TRANSITION_CONFLICT', message: 'Ride transition denied' },
          });
        }
      });
