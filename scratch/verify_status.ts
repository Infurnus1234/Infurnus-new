...
async function main() {
  ...
  if (uRes.rows.length > 0) {
    ...
    const existing = await pool.query('SELECT id FROM driver_profiles WHERE user_id = $1', [
      userId,
    ]);
    ...
    const vCheck = await pool.query('SELECT id FROM vehicles WHERE owner_id = $1 LIMIT 1', [
      userId,
    ]);
